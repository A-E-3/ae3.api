...
public abstract class ExecProcess implements ExecLexicalScope, ExecStackContext/* , ExecArguments */, BaseArray {
	...
	private static final BaseObject arrayReadClean(final BaseObject[] array, final int index) {
	...
	/** FCALLAxxx instruction implementation
	 *
	 ...
	 *
	 * @param constant
	 *            number of arguments on stack (plus 1: callee)
	 * @param resultHandler
	 * @return */
	public final ExecStateCode vmCall_FCALLA_FSS(//
			final int constant,
			final ResultHandler resultHandler //
	) {
		
		if (this.ri0ASP - (constant + 1) < this.ri0BSB) {
			return this.vmRaise("FCALLA: " + (constant + 1) + " stack arguments expected");
		}
		...
		switch (constant) {
			case 0 : {
				final BaseFunction callee = ExecProcess.arrayReadClean(stack, --this.ri0ASP).baseCall();
				final BaseObject instance = this.contextImplicitThisValue();
				if (callee == null) {
					return this.vmRaise("Not a function: class=" + instance.getClass().getName());
				}
				return callee.execCallPrepare(this, instance, resultHandler, false);
			}
			case 1 : {
				final BaseObject argument = ExecProcess.arrayReadClean(stack, --this.ri0ASP);
				final BaseFunction callee = ExecProcess.arrayReadClean(stack, --this.ri0ASP).baseCall();
				final BaseObject instance = this.contextImplicitThisValue();
				if (callee == null) {
					return this.vmRaise("Not a function: class=" + instance.getClass().getName());
				}
				return callee.execCallPrepare(this, instance, resultHandler, false, argument);
			}
			case 2 : {
				final BaseObject argument2 = ExecProcess.arrayReadClean(stack, --this.ri0ASP);
				final BaseObject argument1 = ExecProcess.arrayReadClean(stack, --this.ri0ASP);
				final BaseFunction callee = ExecProcess.arrayReadClean(stack, --this.ri0ASP).baseCall();
				final BaseObject instance = this.contextImplicitThisValue();
				if (callee == null) {
					return this.vmRaise("Not a function: class=" + instance.getClass().getName());
				}
				return callee.execCallPrepare(this, instance, resultHandler, false, argument1, argument2);
			}
			default : {
				
				final ExecArguments arguments = this.argumentsCopy(constant);
				/** have to clean up */
				final int rASP = this.ri0ASP;
				for (int i = constant; i > 0; --i) {
					stack[rASP - i] = null;
				}
				this.ri0ASP -= constant;
				...
				return callee.execCallPrepare(this, instance, resultHandler, false, arguments);
			}
		}
	 ...
	 * @param resultHandler
	 * @return */
	public final ExecStateCode vmCall_Generic_MapArgs(//
	 ...
	 * @param resultHandler
	 * @return */
	public final ExecStateCode vmCall_Generic_StackArgs(//
			final BaseFunction callee,
			final BaseObject thisValue,
			final int count,
			final ResultHandler resultHandler//
	 ...
	 * function implementation should not return RETURN or exception could be raised
	 *
	 * thisObject and ARGUMENTS are ON STACK
	 *
	 * @param accessProperty
	 *            propertyName
	 * @param count
	 *            number of arguments on stack
	 * @param resultHandler
	 * @return */
	public final ExecStateCode vmCall_RCALL_SaS(//
			final BaseObject accessProperty,
			final int count,
			final ResultHandler resultHandler//
	) {
		
		if (count <= 0) {
			return this.vmRaise("vmCallTS: invalid count argument: " + count);
		}
		
		final BaseObject[] stack = this.fldStack;
		final BaseObject thisValue = stack[this.ri0ASP - count];
		
		assert thisValue != this || this == this.rb4CT : "Use FCALLS then";
		
		final BaseObject candidate = thisValue.baseGet(accessProperty, BaseObject.UNDEFINED);
		
		final BaseFunction callee = candidate.baseCall();
		if (callee == null) {
			if (candidate == BaseObject.UNDEFINED) {
				return this.vmRaise(
						Format.Compact.baseObject(thisValue) + " has no property called " //
								+ Format.Compact.baseObject(accessProperty));
			}
			return this.vmRaise("Not a function: key=" + accessProperty.baseToJavaString() + ", class=" + candidate.getClass().getName());
		}
		
		{
			final ExecArguments arguments = this.argumentsCopy(count - 1);
	...
	/** RCALLAxxx instruction implementation
	 *
	 * function implementation should not return RETURN or exception could be raised
	 *
	 * EVERYTHING ON STACK
	 *
	 * @param constant
	 *            number of arguments on stack (plus 2: callee & thisValue)
	 * @param resultHandler
	 * @return */
	public final ExecStateCode vmCall_RCALLA_SSS(//
			final int constant,
			final ResultHandler resultHandler //
	) {
		
		if (this.ri0ASP - (constant + 2) < this.ri0BSB) {
			return this.vmRaise("CALLA: " + (constant + 2) + " stack arguments expected");
		}
		...
		switch (constant) {
			case 0 : {
				final BaseFunction callee = ExecProcess.arrayReadClean(stack, --this.ri0ASP).baseCall();
				final BaseObject instance = ExecProcess.arrayReadClean(stack, --this.ri0ASP);
				if (callee == null) {
					return this.vmRaise("Not a function: class=" + instance.getClass().getName());
				}
				return callee.execCallPrepare(this, instance, resultHandler, false);
