...
public abstract class ExecProcess implements ExecLexicalScope, ExecStackContext/* , ExecArguments */, BaseArray {
	 ...
	 * @param resultHandler
	 * @return */
	public final ExecStateCode vmCallM(//
	 ...
	 * @param resultHandler
	 * @return */
	public final ExecStateCode vmCallTS(final BaseObject argumentB, final int count, final ResultHandler resultHandler) {
		...
		final BaseObject[] stack = this.fldStack;
		final BaseObject argumentA = stack[this.ri0ASP - count];
		
		assert argumentA != this || this == this.rb4CT : "Use FCALLS then";
		...
		if (callee == null) {
			...
			return this.vmRaise("Not a function: key=" + argumentB.baseToJavaString() + ", class=" + candidate.getClass().getName());
		}
		
		{
			...
			return callee.execCallPrepare(this, argumentA, resultHandler, false, arguments);
		}
	}
