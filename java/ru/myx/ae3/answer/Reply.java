...
/**
 * CD_XXX codes represent values of HTTP response codes, you can look up their
 * values and descriptions online
...
public class Reply extends AbstractSAPI {
	 ...
	 *
	 * Status code (504) indicating that the HTTP service was unable to receive
	 * gateway reply within time allotted.
	 */
	 ...
	 *
	 * Status code (507) indicating that the there is 'insufficient storage'
	 * condition.
	 */
	 ...
	 *
	 * Status code (402) indicating that the request cannot be served though
	 * seems valid.
	 */
	 ...
	 * @return reply
	 */
	public static final UniversalReplyAnswer<?> binaryAndCharacter(final String owner,
	...
	public static UniversalReplyAnswer<?> binaryWrapCharacter(final CharacterReplyAnswer<?> characterReply) {
		...
		if (originalAttributes != null) {
			...
			ownContentCharsetAttribute : {
				 ...
				 */
				attributes.baseDefine("Content-Type", (contentType != null
	...
	public static UniversalReplyAnswer<?> characterWrapBinary(final BinaryReplyAnswer<?> binaryReply) {
		...
		final String encoding = currentEncoding.length() > 0
			...
			: originalAttributes == null
				...
				: Base.getString(originalAttributes, "Character-Encoding", binaryReply.getQuery() == null
					? null
					: binaryReply.getQuery().getAttributes(), "Accept-Charset", Engine.ENCODING_UTF8);
		final String chosenEncoding = encoding != Engine.ENCODING_UTF8 && encoding.indexOf('*') != -1
			? Engine.ENCODING_UTF8
			: encoding;
		final BaseObject attributes;
	...
	public static final ReplyAnswer redirect(final String owner, final BaseMessage query, final boolean moved, final String url) {
		...
		return Reply.string(owner, query, url).setCode(moved
			? Reply.CD_MOVED
			: Reply.CD_LOOKAT).setFinal();
	}
