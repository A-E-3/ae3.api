...
public class Storage extends AbstractSAPI {
	...
	public static final <O extends ArsRecord, R extends ArsReference<O>, A extends ArsArray<R>> //
			EntryContainer
					createRoot(final ArsStorageImpl<O, R, A> impl) {
	...
	/**
	 * Returns new root transaction or null when context is already running
	 ...
	 * ), then creates an empty file if it does not exist yet.
	 *
	 * @param root
	 * @param focus
	 * @param path
	...
	/**
	 * Calls getRelative(ctx, path, TreeLinkType.PUBLIC_TREE_REFERENCE ), then
	...
	public static final//
		...
		return root.relative(path, mode);
	}
