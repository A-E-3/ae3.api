...
public class Request extends AbstractSAPI {
	 ...
	 * @return
	 */
	public static BinaryServeRequest<?> binary(
			final String owner,
			final String verb,
	...
	private static UniversalServeRequest<?> binaryAndCharacter(
			final String owner,
			final String verb,
			final BaseObject attributes,
	 ...
	 */
	@Deprecated
	public static void binaryWrapCharacter(
			final BodyAccessBinary character) {
	 ...
	 */
	public static UniversalServeRequest<?> binaryWrapCharacter(
			final CharacterServeRequest<?> characterRequest) throws UnsupportedEncodingException {
		 ...
		 * </code> ...we will enforce that they
		 * checked and handled it explicitly before calling this method
		 */
		...
		final BaseObject tempAttributes = BaseObject.createObject(ownAttributes);
		{
			...
			if (contentType.length() != 0) {
				...
				if (explicitEncoding.length() > 0) {
					...
					return Request.binaryAndCharacter( characterRequest.getEventTypeId(), //
		...
		}
		{
			...
			if (contentCharset.length() > 0) {
				...
				final TransferCopier binary = Transfer.createCopier( text, charset );
				tempAttributes.baseDefine("Content-Type", (contentTypeAttribute != null
		...
		}
		{
			...
			tempAttributes.baseDefine("Content-Charset", Engine.ENCODING_UTF8);
			tempAttributes.baseDefine("Content-Length", binary.length());
	 ...
	 * @return
	 */
	public static CharacterServeRequest<?> character(
			final String owner,
			final String verb,
			final BaseObject attributes,
			final CharSequence text) {
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static UniversalServeRequest<?> characterWrapBinary(
			final BinaryServeRequest<?> binaryRequest) throws UnsupportedEncodingException {
	 ...
	 */
	@Deprecated
	public static void characterWrapBinary(
			final BodyAccessCharacter binary) {
	 ...
	 * @return
	 */
	public static UniversalServeRequest<?> empty(
			final String owner, //
			final String verb) {
	 ...
	 * @return
	 */
	public static final String getDefaultTitleForVerb(
			final String verb) {
	...
	/**
	 * Some static crap. If shields ReplyAnswerExceptions and returns them as
	 * reply object as well.
	 * 
	 * @param ctx
	 * @param handler
	 * @param context
	 ...
	 * @throws Exception
	 */
	public static final BaseObject queryHandler(
			final ExecProcess ctx,
			final BaseObject handler,
			final BaseObject context) throws Exception {
