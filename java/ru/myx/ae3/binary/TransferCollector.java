...
/** A special type of object allowing non-blocking bufferisation between absorption made using
 * Target or OutputStream and retrieval via Source methods. Any collector implementation have to be
 * safe for at least 2-Thread multithreading when one thread writes to a collector and another
 * thread is reading.
 *
...
public interface TransferCollector extends Closeable, TransferSource {
	...
	/** Activates the 'chunking' state on this collector, any data written to a collector will be
	 * divided into chunks whose maximal size is specified as maxChunk argument. minChank argument
	 * should be used as a hint to create some buffers while collecting. Collected data can be read
	 * in form of <i>chunked-encoding </i> (as specified in MIME protocol).
	 *
	 * @param minChunk
	 * @param maxChunk
	 ...
	 *
	 * Returns a factory whose nextSource() method always returns a Buffer object with the same
	 * contents as in this <b>whole </b> Source. After execution of this method source should be in
	 * exhausted state, collector is reset and ready to absorb new data after this method is
	 * returned
	 *
	 * @return copier */
	TransferCopier toBinary();

	/** Returns whole contents of <b>closed </b> collector as a buffer, collector is reset and ready
	 * to absorb new data after this method is returned.
	 *
	 * @return buffer */
