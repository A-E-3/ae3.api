...
public abstract class BaseEditableAbstract extends BaseAbstract implements BaseEditable {
	...
	@Override
	public final boolean setOwnProperty(final BasePrimitiveString name, final BaseProperty property, final short attributes) {

		assert property != null : "NULL property";

		final BaseProperties<?> properties = this.properties;
		if (properties == null) {
			this.properties = BaseProperties.createFirstProperty(this, name, property, attributes);
			return true;
		}

		final BaseProperty existing = properties.find(name);
		if (existing == null) {
			this.properties = properties.add(this, name, property, attributes);
			return true;
		}
		...
		/** will gracefully replace the existing one **/
		this.properties = properties.add(this, name, property, attributes);
		return true;
	}

	@Override
	public final boolean setOwnProperty(final String name, final BaseProperty property, final short attributes) {
