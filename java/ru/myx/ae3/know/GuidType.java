...
enum GuidType {
	...
	NULL {
		@Override
		final BasePrimitiveNull getBaseObject(
				final byte[] bytes) {
	...
	BOOLEAN_FALSE {
		@Override
		final BasePrimitiveBoolean getBaseObject(
				final byte[] bytes) {
	...
	NUMBER_NAN {
		@Override
		final BasePrimitiveNumber getBaseObject(
				final byte[] bytes) {
	...
	NUMBER_NEGATIVE_INT_64BIT {
		...
		@Override
		final long getJavaPrimitiveNumberInteger(
				final byte[] bytes) {
	...
	NUMBER_NEGATIVE_NOT_INTEGER {
		...
		final double getJavaPrimitiveNumberDouble(
				final byte[] bytes) {
		
			return Double.longBitsToDouble( 0xFFFFFFFFFFFFFFFFL
					^ ((long) bytes[1] << 56)
					+ ((long) (bytes[2] & 255) << 48)
					+ ((long) (bytes[3] & 255) << 40)
					+ ((long) (bytes[4] & 255) << 32)
	...
	NUMBER_POSITIVE_NOT_INTEGER {
		...
		final double getJavaPrimitiveNumberDouble(
			...
			return Double.longBitsToDouble( ((long) bytes[1] << 56)
					+ ((long) (bytes[2] & 255) << 48)
					+ ((long) (bytes[3] & 255) << 40)
					+ ((long) (bytes[4] & 255) << 32)
					+ ((long) (bytes[5] & 255) << 24)
	...
	STRING_EMPTY {
		...
		@Override
		final BasePrimitiveString getBaseObject(
				final byte[] bytes) {
	...
	BYTES_EMPTY {
		@Override
		final BaseObject getBaseObject(
				final byte[] bytes) {
		...
		@Override
		final TransferCopier getJavaObject(
				final byte[] bytes) {
	...
	/**
	 * For all immutable character data with it's binary length stored as 1
	 * byte, whirlpool-based.
	 * 
	 * Format: guid-type byte, then 48 bytes of checksum.
	 */
	TXT384_8BIT_SZ {
	...
	/**
	 * For all immutable character data with it's binary length stored as 2
	 * bytes, whirlpool-based.
	 * 
	 * Format: guid-type byte, then 48 bytes of checksum.
	 */
	TXT384_16BIT_SZ {
	...
	/**
	 * For all immutable character data with it's binary length stored as 3
	 * bytes, whirlpool-based.
	 * 
	 * Format: guid-type byte, then 48 bytes of checksum.
	 */
	...
	GUID_RESERVED {
		...
		@Override
		final BasePrimitiveUndefined getBaseObject(
				final byte[] bytes) {
	...
	public static final byte[] createCRC(
			final byte[] binary,
			final long length) {
	...
	static final byte[] createCRC(
			final WhirlpoolDigest digest,
			final long length) {
	...
	public static final byte[] createTXT(
			final byte[] binary,
			final long length) {
	 ...
	 * @return
	 */
	public static final boolean isInlineTypeForOrdinal(
			final int guidTypeByte) {
	...
	long getJavaPrimitiveNumberInteger(
			final byte[] bytes) {
